"""
Phase 2 - kNN with Permutation Importance Feature Selection
===========================================================
Feature selection is computed ONCE per fold using Random Forest permutation
importance, then kNN is tuned on reduced features.
...
Process per fold:
  ...
  4. Tune kNN on reduced features, evaluate on reduced test fold
...
DATA_DIR = "data"
RESULTS_DIR = "results/phase2/knn"
TARGET_COL = "Label"
RANDOM_STATE = 42
...
def evaluate_dataset(csv_path, dataset_name):
    ...
    print(f"\n  [KNN] Acc: {acc_mean:.4f} ± {acc_std:.4f} | "
...
def save_results(folds_df, summary_df, dataset_name):
    folds_path = os.path.join(RESULTS_DIR, f"{dataset_name}_knn_p2_folds.csv")
    summary_path = os.path.join(
        RESULTS_DIR, f"{dataset_name}_knn_p2_summary.csv")
...
def combine_all_summaries(frames):
    if not frames:
        return
    out_path = os.path.join(RESULTS_DIR, "all_datasets_knn_p2_summary.csv")
