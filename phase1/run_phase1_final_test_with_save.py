...
RESULTS_DIR = os.path.join("results", "final_test_saved", "phase1")
...
def load_phase1_best_hyperparameters(path: str) -> Dict[str, Dict[str, Any]]:
    ...
    phase1_df = df[df["Phase"].astype(
        str).str.strip().eq("Before FS/DR")].copy()
    if phase1_df.empty:
        raise ValueError(
            "No 'Before FS/DR' rows found in hyperparameter table")
...
def run_classifier(classifier: str, params: Dict[str, Any]) -> pd.DataFrame:
    ...
    print(f"Phase     : Before FS/DR (WITH MODEL SAVING)")
    ...
    out_path = os.path.join(
        RESULTS_DIR, f"all_phase1_{classifier.lower()}_final_test_results.csv")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Final Phase 1 test-set evaluation using chosen best hyperparameters, WITH MODEL SAVING."
    )
    parser.add_argument(
        "--classifier",
...
def main() -> None:
    ...
    for classifier in classifiers:
        if classifier not in phase1_params:
            raise ValueError(
                f"No Phase 1 hyperparameters found for classifier: {classifier}")
        all_frames.append(run_classifier(
            classifier, phase1_params[classifier]))

    combined_df = pd.concat(all_frames, ignore_index=True)
    combined_path = os.path.join(
        RESULTS_DIR, "all_phase1_final_test_results.csv")
