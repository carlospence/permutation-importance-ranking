"""
Phase 2 - Interactive All Classifiers with Permutation Importance Feature Selection
====================================================================================
...
DATA_DIR = "data"
RESULTS_DIR = "results/interactive/phase2"
TARGET_COL = "Label"
RANDOM_STATE = 42
RBF_CLASS_LIMIT = 10
SGD_CLASS_LIMIT = 10
...
def evaluate_dataset(csv_path: str, dataset_name: str, classifiers: List[str]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Evaluate selected classifiers on a single dataset with permutation importance feature selection."""
    ...
    rows = {clf: [] for clf in classifiers}
    ...
    for fold, (train_idx, test_idx) in enumerate(outer_cv.split(X, y), start=1):
        ...
        if "knn" in classifiers:
            t = time.time()
            est, p = run_grid(get_knn_pipeline(),
                              get_knn_param_grid(), X_tr, y_train, inner_cv)
            ...
            print(
                f"    kNN  | Acc={acc:.4f} F1={f1:.4f} | {round(time.time()-t, 1)}s")
        ...
        if "rf" in classifiers:
            t = time.time()
            est, p = run_grid(get_rf_pipeline(),
                              get_rf_param_grid(), X_tr, y_train, inner_cv)
            y_pred = est.predict(X_te)
            acc = round(accuracy_score(y_test, y_pred), 4)
            f1 = round(f1_score(y_test, y_pred,
                       average="macro", zero_division=0), 4)
            ...
            rows["rf"].append({**base_row(), "Accuracy": acc, "F1": f1,
                               ...
                               "Parameters": str(cp), "Fold_Time_Seconds": round(time.time()-t, 2)})
            print(
                f"    RF   | Acc={acc:.4f} F1={f1:.4f} | {round(time.time()-t, 1)}s")
...
def save_results(folds_df: pd.DataFrame, summary_df: pd.DataFrame, dataset_name: str, classifiers: List[str]) -> None:
    """Save combined and per-classifier results."""
    clf_suffix = "_".join(classifiers)

    # Save combined
    folds_path = os.path.join(
        RESULTS_DIR, f"interactive_{dataset_name}_{clf_suffix}_folds.csv")
    summary_path = os.path.join(
        RESULTS_DIR, f"interactive_{dataset_name}_{clf_suffix}_summary.csv")
...
def display_menu() -> None:
    """Display main menu."""
    print("\n" + "="*80)
    print("Phase 2: Interactive Classifier Evaluation with Permutation Importance")
    print("="*80)
    print("\nUsage:")
    print("  - Enter dataset numbers as: 1 or 1,5,10 or 1-5 (range)")
    print("  - Enter classifier names as: svm or svm,dt or dt,knn,rf")
    print("  - Available classifiers: svm, knn, dt, rf, mlp")
    print("="*80)
...
def parse_classifiers(user_input: str) -> List[str]:
    ...
    try:
        ...
        valid_selected = [c for c in selected if c in valid_classifiers]

        if not valid_selected:
            print(
                f"❌ No valid classifiers selected. Available: {', '.join(valid_classifiers)}")
            return []

        if len(valid_selected) < len(selected):
            invalid = [c for c in selected if c not in valid_classifiers]
...
def main():
    ...
    while True:
        ...
        print(f"\n🤖 Available classifiers: svm, knn, dt, rf, mlp")
        classifier_input = input(
            "\n📌 Enter classifier(s) [e.g., svm or svm,dt or dt,knn,rf]: ").strip()
        ...
        for folder in selected_datasets:
            ...
            try:
                folds_df, summary_df = evaluate_dataset(
        ...
        if all_summaries:
            ...
            combined_path = os.path.join(
                RESULTS_DIR,
                f"interactive_combined_{clf_suffix}.csv"
            )
