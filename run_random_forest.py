...
def get_rf_param_grid():
    """
    Tuning n_estimators and max_depth as specified in the instructions.
    - n_estimators: more trees = more stable but slower. 100/200 is a safe range.
    - max_depth: None = fully grown trees. Limiting depth speeds up training
      and reduces overfitting on imbalanced datasets like 9-16.
    - min_samples_split: extra regularisation for imbalanced classes.
    """
    return {
...
def evaluate_dataset(csv_path: str, dataset_name: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for fold, (train_idx, test_idx) in enumerate(outer_cv.split(X, y), start=1):
        ...
        clean_params = {k.replace("rf__", ""): v for k, v in best_params.items()}
        ...
        fold_rows.append({
            ...
            "Accuracy":               accuracy,
            "F1":                     f1_macro,
            "Best_N_Estimators":      clean_params.get("n_estimators"),
            "Best_Max_Depth":         clean_params.get("max_depth"),
            "Best_Min_Samples_Split": clean_params.get("min_samples_split"),
...
def save_results(folds_df: pd.DataFrame, summary_df: pd.DataFrame, dataset_name: str) -> None:
    folds_path   = os.path.join(RESULTS_DIR, f"{dataset_name}_rf_folds.csv")
    summary_path = os.path.join(RESULTS_DIR, f"{dataset_name}_rf_summary.csv")
...
def combine_all_summaries(frames: list[pd.DataFrame]) -> None:
    if not frames:
        return
    out_path = os.path.join(RESULTS_DIR, "all_datasets_rf_summary.csv")
