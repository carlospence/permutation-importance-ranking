...
def load_phase2_best_hyperparameters(path: str) -> Dict[str, Dict[str, Any]]:
    ...
    phase2_df = df[df["Phase"].astype(str).str.strip().eq("After FS/DR")].copy()
    if phase2_df.empty:
...
def run_classifier(classifier: str, params: Dict[str, Any]) -> pd.DataFrame:
    ...
    out_path = os.path.join(RESULTS_DIR, f"all_phase2_{classifier.lower()}_final_test_results.csv")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Final Phase 2 test-set evaluation using chosen best hyperparameters and permutation-importance feature selection."
    )
    parser.add_argument(
        "--classifier",
...
def main() -> None:
    ...
    for classifier in classifiers:
        if classifier not in phase2_params:
            raise ValueError(f"No Phase 2 hyperparameters found for classifier: {classifier}")
        all_frames.append(run_classifier(classifier, phase2_params[classifier]))

    combined_df = pd.concat(all_frames, ignore_index=True)
    combined_path = os.path.join(RESULTS_DIR, "all_phase2_final_test_results.csv")
