"""
Phase 2 - Random Forest with Permutation Importance Feature Selection
====================================================================
Feature selection is computed ONCE per fold using Random Forest permutation
importance, then Random Forest is tuned on reduced features.
...
Process per fold:
  ...
  4. Tune RF on reduced features, evaluate on reduced test fold
...
# ══════════════════════════════════════════════════════════════════════════════
#  RANDOM FOREST CLASSIFIER DEFINITIONS
# ══════════════════════════════════════════════════════════════════════════════
...
def save_results(folds_df, summary_df, dataset_name):
    folds_path = os.path.join(RESULTS_DIR, f"{dataset_name}_rf_p2_folds.csv")
    summary_path = os.path.join(
        RESULTS_DIR, f"{dataset_name}_rf_p2_summary.csv")
...
def combine_all_summaries(frames):
    if not frames:
        return
    out_path = os.path.join(RESULTS_DIR, "all_datasets_rf_p2_summary.csv")
